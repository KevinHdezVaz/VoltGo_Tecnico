  ...
  static Future<ApiResult> updateTechnicianProfile({
    ...
    static Future<AppleSignInResult> loginWithApple() async {
      try {
  ...
  // Método para enviar datos de Apple al backend
    static Future<AppleSignInResult> _sendAppleTokenToBackend({
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          return AppleSignInResult(success: false, error: errorMessage);
        }
      } catch (e) {
  ...
  static Future<bool> isAppleSignInAvailable() async {
      try {
        ...
        // Verificar disponibilidad usando el plugin
    ...
    // NUEVO: Método para registro con Apple Sign-In
static Future<AppleSignInResult> registerWithApple() async {
  // Por ahora, usar el mismo flujo que loginWithApple
  // ya que el backend maneja la creación automática de usuarios
